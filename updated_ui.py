...
def add_level(default_value="YYYY"):
    ...
    levels_frame = ttk.Frame(levels_container, padding="5")
    levels_frame.grid(sticky=(tk.W, tk.E), pady=5)
    ttk.Label(levels_frame, text="Level:").grid(row=0, column=0, sticky=tk.W, padx=5)
    ...
    level_dropdown.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=5)
    ttk.Button(levels_frame, text="Remove", command=levels_frame.destroy).grid(row=0, column=2, sticky=tk.W, padx=5)
...
def start_processing():
    ...
    duplicates_dir = destination_dir / duplicates_dir_entry.get()
    unknown_dir = destination_dir / unknown_dir_entry.get()
...
ttk.Label(frame, text="Photo Organizer", font=("Arial", 16, "bold")).grid(row=0, column=0, columnspan=3, pady=(0, 10))
ttk.Label(frame, text="Organize and rename your photos efficiently").grid(row=1, column=0, columnspan=3, pady=(0, 20))

ttk.Label(frame, text="Source Directory:").grid(row=2, column=0, sticky=tk.W, padx=5)
source_dir_entry = ttk.Entry(frame, width=50)
source_dir_entry.grid(row=2, column=1, sticky=(tk.W, tk.E))
ttk.Button(frame, text="Browse", command=lambda: source_dir_entry.delete(0, tk.END) or source_dir_entry.insert(0, filedialog.askdirectory())).grid(row=2, column=2, sticky=tk.W, padx=5)

ttk.Label(frame, text="Destination Directory:").grid(row=3, column=0, sticky=tk.W, padx=5)
dest_dir_entry = ttk.Entry(frame, width=50)
dest_dir_entry.grid(row=3, column=1, sticky=(tk.W, tk.E))
ttk.Button(frame, text="Browse", command=lambda: dest_dir_entry.delete(0, tk.END) or dest_dir_entry.insert(0, filedialog.askdirectory())).grid(row=3, column=2, sticky=tk.W, padx=5)

ttk.Label(frame, text="Folder Structure Levels:").grid(row=4, column=0, sticky=tk.W, padx=5)
levels_container = ttk.Frame(frame)
levels_container.grid(row=5, column=0, columnspan=3, sticky=(tk.W, tk.E))
ttk.Button(frame, text="Add Level", command=add_level).grid(row=6, column=0, sticky=tk.W, padx=5)
level_vars = []
...
ttk.Label(frame, text="File Renaming Strategy:").grid(row=7, column=0, sticky=tk.W, padx=5)
...
rename_strategy_dropdown.grid(row=7, column=1, sticky=(tk.W, tk.E), padx=5)

ttk.Label(frame, text="Select File Types:").grid(row=8, column=0, sticky=tk.W, padx=5)
filetype_vars = {}
filetypes = [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".tiff", ".raw", ".cr2", ".nef", ".orf", ".sr2", ".heic", ".arw", ".mp4", ".mov", ".avi", ".mkv", ".mts", ".m2ts", ".3gp"]

filetype_frame = ttk.Frame(frame, padding="5")
filetype_frame.grid(row=9, column=0, columnspan=3, sticky=(tk.W, tk.E))
for i, ext in enumerate(filetypes):
    var = tk.BooleanVar(value=True)
    chk = ttk.Checkbutton(filetype_frame, text=ext, variable=var)
    chk.grid(row=i//5, column=i%5, sticky=tk.W, padx=5)
    filetype_vars[ext] = var

ttk.Label(frame, text="Duplicates Folder Location:").grid(row=10, column=0, sticky=tk.W, padx=5)
duplicates_dir_entry = ttk.Entry(frame, width=50)
duplicates_dir_entry.grid(row=10, column=1, sticky=(tk.W, tk.E))
duplicates_dir_entry.insert(0, "Duplicates")  
ttk.Button(frame, text="Browse", command=lambda: duplicates_dir_entry.delete(0, tk.END) or duplicates_dir_entry.insert(0, filedialog.askdirectory())).grid(row=10, column=2, sticky=tk.W, padx=5)

ttk.Label(frame, text="Unknown Folder Location:").grid(row=11, column=0, sticky=tk.W, padx=5)
unknown_dir_entry = ttk.Entry(frame, width=50)
unknown_dir_entry.grid(row=11, column=1, sticky=(tk.W, tk.E))
unknown_dir_entry.insert(0, "Unknown") 
ttk.Button(frame, text="Browse", command=lambda: unknown_dir_entry.delete(0, tk.END) or unknown_dir_entry.insert(0, filedialog.askdirectory())).grid(row=11, column=2, sticky=tk.W, padx=5)
...
delete_empty_chk.grid(row=12, column=0, columnspan=3, sticky=tk.W, pady=10)


progress_var = tk.DoubleVar()
progress_bar = ttk.Progressbar(frame, variable=progress_var, maximum=100)
progress_bar.grid(row=13, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)


progress_label = ttk.Label(frame, text="Ready to start")
progress_label.grid(row=14, column=0, columnspan=3, sticky=(tk.W, tk.E))


start_button = ttk.Button(frame, text="Start", command=start_processing)
start_button.grid(row=15, column=0, sticky=tk.W, padx=5, pady=10)


exit_button = ttk.Button(frame, text="Exit", command=exit_application)
exit_button.grid(row=15, column=2, sticky=tk.E, padx=5, pady=10)
